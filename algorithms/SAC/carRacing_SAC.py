...
# Custom Callback untuk rekam reward
...
# 1) Buat environment (tanpa render)
vec_env = DummyVecEnv([lambda: gym.make('CarRacing-v3', render_mode=None)])

# 2) Model SAC
model = SAC('CnnPolicy', vec_env,buffer_size=100000, verbose=1, tensorboard_log="./sac_car_racing_tb/")
...
# 5) Save hasil ke CSV
save_dir = 'csv'
base_filename = 'sac_reward_vs_timesteps'
extension = '.csv'
os.makedirs(save_dir, exist_ok=True)

# Cari nama file otomatis
counter = 0
while True:
    if counter == 0:
...
plt.title('SAC on CarRacing-v3: Reward vs Timesteps')
plt.legend()
plt.grid()
plt.tight_layout()
plt.show()
...
sample_efficiency = avg_reward / (total_timesteps / 1000)
stability = np.std(df['Episode_Reward'])

# 8) Cetak hasil
print("\n==== Training Summary (SAC) ====")
...
metrics_save_path = 'csv/sac_metrics_performance.csv'
...
model_base_name = 'sac_car_racing_vec'
...
print(f"\n✅ Model SAC disimpan di {model_filepath}")

# 10) Render hasil training
print("\n🎬 Mulai rendering dari model terlatih (SAC)...")
env_display = gym.make('CarRacing-v3', render_mode="human")
model_loaded = SAC.load(model_filepath)
